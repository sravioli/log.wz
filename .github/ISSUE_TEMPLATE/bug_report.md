...
- **log.wz version/commit**: (e.g. `v1.0.0` or commit hash)
- **Installation method**: git (`wezterm.plugin.require`) / local checkout
- **Lua version** (if known):

## Setup

Provide your `log:setup` call and logger creation code so the issue can be
reproduced.

```lua
-- Example:
-- log:setup { threshold = "DEBUG", sinks = { default_enabled = true } }
-- local logger = log:new("my-tag", true, { log.sinks.file() })
```
