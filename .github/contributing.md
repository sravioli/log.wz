# log.wz Contributing Guide

Welcome to the log.wz contributing guide, and thank you for your interest.

We accept the following types of contributions:

- **Source code** — bug fixes, new features, and improvements to existing sinks
  or the logging core. See [Environment setup](#environment-setup), [Best
...
log.wz is a logging library for [WezTerm](https://wezfurlong.org/wezterm/)
plugins and configuration code. It provides tagged logger instances, level-based
filtering, and pluggable sinks (WezTerm native, JSON, file, in-memory ring
buffer). See the [README](../README.md) for full usage details.
...
1. Check existing [issues](https://github.com/sravioli/log.wz/issues) to avoid
   duplicates.
...
2. Clone your fork:
   ...
   ```sh
   git clone https://github.com/<your-username>/log.wz.git
   cd log.wz
   ```
...
3. To test the plugin locally, point your WezTerm config at the checkout:
   ...
   ```lua
   local log = wezterm.plugin.require(
     "file:///" .. wezterm.config_dir .. "/plugins/log.wz"
   )
   ```
...
- **Isolate sink errors.** Every sink runs inside `pcall`. If you add a new sink
  or modify an existing one, make sure it doesn't propagate errors to the
  caller.
- **Test in WezTerm.** There is no automated test suite. Manually verify your
  changes in WezTerm's debug overlay and, when relevant, with the file and
  memory sinks.
...
Include your OS, WezTerm version, log.wz version or commit, your `log:setup`
call, and steps to reproduce the bug.
...
```text
fix(file-sink): prevent write inside config_dir
feat(memory): add max_entries option
...
2. Open a pull request against `main` on
   [sravioli/log.wz](https://github.com/sravioli/log.wz).
...
5. A maintainer will review the PR. If you don't receive feedback within a
